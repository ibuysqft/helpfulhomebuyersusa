...
export function CredibilityBar() {
  const { stateName, stateAbbr, isNational } = siteConfig
  const displayRegion = isNational ? 'Nationwide' : stateName
  ...
  const badges = [
    ...
    },
    {
      ...
      label: `Licensed & Insured in ${stateAbbr}`,
    },
    {
    ...
    <section className="bg-slate-900 border-y border-white/[0.06] py-6 px-4">
      <div className="max-w-5xl mx-auto">
        <p className="text-center text-xs text-slate-500 uppercase tracking-widest mb-4 font-semibold">
          Trusted By {displayRegion} Homeowners
        </p>
