...
const VALID_CONDITIONS = ['Good', 'Fair', 'Poor', 'Very Poor'] as const
...
export async function POST(req: NextRequest) {
  ...
  const { address, phone, condition, website } = body

  // Honeypot
  if (website) return NextResponse.json({ error: 'Invalid submission' }, { status: 400 })

  if (!address?.trim() || !phone?.trim() || !VALID_CONDITIONS.includes(condition as any)) {
